...
**蓉游智体（Chengdu Travel AI Agent）** 是一个围绕 **成都本地旅游场景** 构建的多智能体 AI Agent 应用，目标不是只回答“成都有什么好玩的”，而是把游客从 **信息查询 → 行程规划 → 避坑决策 → 周边探索 → 天气出行** 这一整条旅行决策链放进一次自然语言交互中完成。

项目以成都真实旅游需求为核心场景，围绕 **大熊猫基地、宽窄巷子、武侯祠、锦里、杜甫草堂、金沙遗址、都江堰、青城山** 等成都及周边典型目的地组织知识、规则、通勤和空间数据。用户既可以查询景点门票、开放时间、历史文化，也可以直接提出“帮我规划成都三日游”“熊猫基地附近还有什么值得去”“今天成都天气适合去青城山吗”这类更复杂的组合需求。

与普通旅游聊天机器人不同，蓉游智体不是把所有问题交给一个 LLM Prompt，而是基于 **LangGraph Supervisor-Worker** 架构构建 Agent 工作流：Supervisor 先识别用户意图，再按任务动态调度 QA、Plan、Advice、Nearby 等专长 Worker；不同 Worker 分别使用 RAG、MySQL 规则数据、空间距离算法和外部天气 API 完成任务，最后通过共享 State 汇总为统一回答。

因此，这个项目更接近一个 **“成都旅游任务执行 Agent”**：LLM 负责理解用户真正想做什么，Agent 负责选择能力和组织执行流程，RAG / 数据库 / 算法 / Tool 负责提供可验证的数据和确定性结果。

### 🐼 为什么是成都旅游 Agent？

- 🏙️ **城市级垂直场景**：项目不是做泛旅游百科，而是围绕成都及周边旅行需求组织数据和能力。
- 🐼 **成都特色景点知识**：知识库覆盖成都代表性景点的介绍、攻略、开放信息、文化背景和旅行提示。
- 🗺️ **成都本地行程规则**：规划阶段结合熊猫基地、都江堰 / 青城山、武侯祠 / 锦里、杜甫草堂 / 金沙遗址等本地路线关系进行约束。
- 🚇 **真实通勤与空间关系**：通过 MySQL 通勤矩阵、景点经纬度、Bounding Box 与 Haversine 计算处理“怎么排”“离多远”“附近有什么”。
- 🌤️ **成都实时天气能力**：通过高德天气 API 获取实时或未来天气，再转换为旅游出行建议。
- 📚 **成都本地数据底座**：景点 API 面向约 1,554 条成都景点数据，并通过 Milvus + MySQL 分别承载非结构化知识与结构化业务信息。

### 🤖 为什么它是一个 Agent，而不是普通聊天机器人？
...
用户自然语言需求
        ↓
Supervisor 理解意图 / 拆分任务
        ↓
选择一个或多个 Worker
        ↓
RAG / SQL / Rules / Spatial Algorithm / Weather Tool
        ↓
Reducer 汇总多个执行结果
        ↓
Supervisor 统一组织答案
...
- 🧭 **自主路由**：Supervisor 根据问题动态判断需要 QA、Plan、Advice、Nearby 中的哪些能力。
- ⚡ **多任务并行**：一个问题可以通过 `Send` 同时派发多个 Worker，而不是固定串行执行。
- 🧩 **能力组合**：不同 Worker 使用不同工具、数据源和算法，不把所有能力堆进一个大 Prompt。
- 🔄 **状态协作**：Worker 通过共享 `MultiAgentState` 写回结果，由 Reducer 解决并行结果合并问题。
- 🛠️ **工具与数据调用**：Agent 可以调用 RAG、MySQL、Haversine、天气 API 等外部能力完成任务。
- 🛡️ **确定性校验**：行程规则、空间距离、JSON 解析等关键逻辑交给程序处理，而不是完全相信模型生成。
- 🧠 **统一汇总**：多个专项 Worker 执行完后，由 Supervisor 根据原始问题重新组织成完整旅行回答。

### 💬 典型使用场景

- 📖 **景点问答**  
  “成都大熊猫基地几点开放？第一次去有什么注意事项？”  
  → QA Worker → Hybrid RAG → 检索成都景点知识 → 生成回答。

- 🗺️ **多日行程**  
  “第一次去成都，帮我规划一个三日游，希望包含熊猫基地、武侯祠和都江堰。”  
  → Plan Worker → MySQL Rules / Transit → LLM JSON → `validate_plan()`。

- 🧩 **复合旅行需求**  
  “帮我规划成都两日游，再告诉我有哪些容易踩坑的地方。”  
  → Supervisor 同时调度 Plan + Advice → `Send` 并行 → Reducer 合并 → Summary。

- 📍 **周边探索**  
  “宽窄巷子附近 3 公里还有什么景点？”  
  → Nearby Worker → 参数提取 → MySQL 坐标 → Bounding Box → Haversine → Top10。

- 🌤️ **实时出行决策**  
  “成都今天下雨吗？适合去熊猫基地吗？”  
  → Weather Tool → 高德天气 API → LLM 整理为旅游建议。

### 🎯 项目希望解决的问题

- 📚 **旅游信息分散**：景点介绍、票价、开放时间、攻略等信息通常需要在多个平台之间反复查询。
- 🗺️ **行程规划复杂**：多日路线不仅是景点列表，还要考虑通勤、闭馆日、景点组合、游玩强度与不同人群需求。
- 📍 **周边关系需要真实计算**：“附近有什么”属于空间问题，不应该依赖 LLM 凭记忆猜测。
- 🧩 **用户需求经常是多意图的**：一次旅行咨询可能同时包含问答、规划、天气和避坑需求。
- 🐼 **通用模型缺乏成都本地业务约束**：需要通过成都知识库、规则数据和本地空间数据增强模型。
- ⚡ **复杂 Agent 链路响应较慢**：需要通过 Worker 并行调度与 SSE 流式返回降低等待感。
- 🛡️ **LLM 输出具有不确定性**：关键规则、JSON、距离和数据库结果需要程序级兜底。

### 🧠 核心设计思想

- 🤖 **LLM 负责理解与生成**：意图识别、自然语言理解、参数抽取和最终内容组织。
- ⚙️ **程序负责确定性逻辑**：SQL 查询、Haversine、JSON 解析、规则校验。
- 🔍 **事实优先来自外部数据**：RAG、MySQL、天气 API，而不是模型自身记忆。
- 🧱 **不同旅游任务拆成不同 Worker**：每个 Worker 使用最适合自己的数据源和算法。
- 🔄 **Agent 通过共享 State 协作**：Worker 不直接互相调用，由 LangGraph 统一编排。
- 🏙️ **通用能力与成都本地知识结合**：Agent 架构负责通用任务编排，成都知识和规则负责垂直场景落地。

### 🌟 核心能力

| 能力 | 处理方式 | 关键实现 |
|---|---|---|
| 📖 **成都旅游问答** | Hybrid RAG | Milvus + BM25 + BGE Reranker + Dynamic Prompt |
| 🗺️ **成都行程规划** | 生成 + 校验 | MySQL Rules + LLM JSON + `validate_plan()` |
| 💡 **避坑建议** | 规则增强生成 | `avoid_rules` + Advice Prompt + LLM |
| 📍 **周边景点** | 空间计算 | LLM 参数抽取 + Bounding Box + Haversine |
| 🌤️ **成都天气查询** | 外部 Tool | 高德天气 API + LLM 整理 |
